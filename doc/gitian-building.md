...
*Setup instructions for a gitian build of Guapcoin Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Guapcoin
...
to the Guapcoin GitHub release page.
...
- [Building Guapcoin Core](#building-guapcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/guapcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/guapcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/guapcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/guapcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/guapcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/guapcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/guapcoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@guapcoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/guapcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/guapcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/guapcoin-osx-*-build.assert
```
