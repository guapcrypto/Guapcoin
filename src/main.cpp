...
/** Fees smaller than this (in uguap) are considered zero fee (for relaying, mining and transaction creation)
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight > 1000000 && nHeight <= 1260000) {
		nSubsidy = 12.5 * COIN;
	}
	else if (nHeight > 1260000 && nHeight <= 1350000) {
		nSubsidy = 25 * COIN;
	}
	else if (nHeight > 1350000 && nHeight <= 2000000) {
		nSubsidy = 21 * COIN;
	}
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> guapInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                guapInputs.push_back(stakeIn);
        }
        const bool hasGUAPInputs = !guapInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasGUAPInputs) {
                    ...
                    for (const CTxIn& guapIn : guapInputs)
                        if(guapIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasGUAPInputs) {
                            for (const CTxIn& stakeIn : guapInputs)
