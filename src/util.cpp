...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "guapcoin" is a composite category enabling all Guapcoin-related debug output
            if (ptrCategory->count(std::string("guapcoin"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "guapcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "guapcoind.pid"));
