...
class ReorgStakeTest(GuapcoinTestFramework):
    ...
    def check_money_supply(self, expected_bc, expected_zguap):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected GUAP and zGUAP supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_bc))
            for denom in node['zGUAPsupply']:
                assert_equal(node['zGUAPsupply'][denom], DecimalAmt(expected_zguap[denom]))
    ...
    def run_test(self):
        ...
        # Check GUAP and zGUAP supply at the beginning
        # ------------------------------------------
        # zGUAP supply: 2 coins for each denomination
        expected_zguap_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # GUAP supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zguap_supply)
        ...
        # Verify that GUAP and zGUAP supplies were properly updated after the spends and reorgs
        self.log.info("Check GUAP and zGUAP supply...")
        ...
        expected_zguap_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zguap_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zguap_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zguap_supply)
