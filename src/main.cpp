...
#if defined(NDEBUG)
#error "Guapcoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uguapcoin) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateGUAPSupply(int nHeightStart)
{
	...
	uiInterface.ShowProgress(_("Recalculating GUAP supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating GUAP supply..."), percent);
		}
...
{
    util::ThreadRename("guapcoin-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> guapcoinInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                guapcoinInputs.push_back(stakeIn);
        }
        const bool hasGUAPInputs = !guapcoinInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasGUAPInputs) {
                    ...
                    for (const CTxIn& guapcoinIn : guapcoinInputs)
                        if(guapcoinIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasGUAPInputs) {
                            for (const CTxIn& stakeIn : guapcoinInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // Guapcoin: We use certain sporks during IBD, so check to see if they are
