...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
		...
		auto guapAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);

		if (!IsValidDestination(guapAdd) || isStakingAddress) {
        ...
        if (walletModel->updateAddressBookLabels(guapAdd, label.toUtf8().constData(),
