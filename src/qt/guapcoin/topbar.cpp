...
#include "qt/guapcoin/topbar.h"
#include "qt/guapcoin/forms/ui_topbar.h"
#include "qt/guapcoin/lockunlock.h"
#include "qt/guapcoin/qtutils.h"
#include "qt/guapcoin/receivedialog.h"
#include "qt/guapcoin/loadingdialog.h"
...
TopBar::TopBar(GuapcoinGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopGuap}, "amount-small-topbar");
    setCssProperty({ui->labelAmountGuap}, "amount-topbar");
    setCssProperty({ui->labelPendingGuap, ui->labelImmatureGuap}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("Guapcoin")
    updateDisplayUnit();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // GUAP Total
    QString totalGuap = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);

    // GUAP
    // Top
    ui->labelAmountTopGuap->setText(totalGuap);
    // Expanded
    ui->labelAmountGuap->setText(totalGuap);
    ui->labelPendingGuap->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureGuap->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
