...
#include "qt/guapcoin/guapcoingui.h"
...
#include "qt/guapcoin/splash.h"
#include "qt/guapcoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("guapcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in guapcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in guapcoin.qrc)
...
/** Class encapsulating Guapcoin startup and shutdown.
...
/** Main Guapcoin application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (GuapcoinGUI)
...
{
    window = new GuapcoinGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &GuapcoinGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &GuapcoinGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(GuapcoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(GuapcoinGUI::DEFAULT_WALLET);
        ...
        // Guapcoin: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &GuapcoinGUI::handlePaymentRequest);
        connect(window, &GuapcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. Guapcoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(guapcoin_locale);
    Q_INIT_RESOURCE(guapcoin);
    ...
    /// 6. Determine availability of data directory and parse guapcoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Guapcoin"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Guapcoin"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // guapcoin: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Guapcoin didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
