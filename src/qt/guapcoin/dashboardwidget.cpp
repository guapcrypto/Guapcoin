...
#include "qt/guapcoin/dashboardwidget.h"
#include "qt/guapcoin/forms/ui_dashboardwidget.h"
#include "qt/guapcoin/sendconfirmdialog.h"
#include "qt/guapcoin/txrow.h"
...
DashboardWidget::DashboardWidget(GuapcoinGUI* parent) :
	PWidget(parent),
	ui(new Ui::DashboardWidget)
{
	...
	// Staking Information
	ui->labelMessage->setText(tr("Amount of GUAP staked."));
	...
	ui->labelAmountGuap->setText("0 GUAP");
	setCssProperty(ui->labelAmountGuap, "text-stake-guap-disable");
	...
	ui->btnHowTo->setText(tr("How to get GUAP"));
...
void DashboardWidget::loadWalletModel() {
	...
	// update the display unit, to not use the default ("GUAP")
	updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
	...
	for (int j = range.first; j < range.second; j++) {
		...
		if (chartData->amountsByCache.contains(num)) {
			...
			guap = (pair.first != 0) ? pair.first / 100000000 : 0;
			chartData->totalGuap += pair.first;
		}

		chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

		chartData->valuesGuap.append(guap);

		int max = std::max(guap, guap);
...
void DashboardWidget::onChartRefreshed() {
	...
	set0->append(chartData->valuesGuap);

	// Total
	nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
	if (chartData->totalGuap > 0) {
		setCssProperty(ui->labelAmountGuap, "text-stake-guap");
	}
	else {
		setCssProperty(ui->labelAmountGuap, "text-stake-guap-disable");
	}
	forceUpdateStyle({ ui->labelAmountGuap });
	ui->labelAmountGuap->setText(GUIUtil::formatBalance(chartData->totalGuap, nDisplayUnit));
