...
const char * const Guapcoin_CONF_FILENAME = "guapcoin.conf";
const char * const Guapcoin_PID_FILENAME = "guapcoin.pid";
const char * const Guapcoin_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Guapcoin only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Guapcoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Guapcoin
// Mac: ~/Library/Application Support/Guapcoin
// Unix: ~/.guapcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Guapcoin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GuapcoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GuapcoinParams
    // Mac: ~/Library/Application Support/GuapcoinParams
    // Unix: ~/.guapcoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GuapcoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "GuapcoinParams";
#else
    // Unix
    return pathRet / ".guapcoin-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", Guapcoin_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", Guapcoin_MASTERNODE_CONF_FILENAME));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty guapcoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override guapcoin.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", Guapcoin_PID_FILENAME));
