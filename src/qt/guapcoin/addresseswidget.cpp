...
#include "qt/guapcoin/forms/ui_addresseswidget.h"
#include "qt/guapcoin/addresslabelrow.h"
#include "qt/guapcoin/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(GuapcoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter Guapcoin address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress guapAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(guapAdd.Get(), label.toUtf8().constData(),
                guapAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
