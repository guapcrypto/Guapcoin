...
- [Development guidelines](#development-guidelines)
    - [General Guapcoin Core](#general-guapcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Guapcoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play GUAPs (tGUAP)" on the test network, if you
...
Guapcoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/guapcoin-project/guapcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_guapcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_guapcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/guapcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_guapcoin.coverage/index.html`.
```
...
- BitcoinMiner : Generates GUAPs (if wallet is enabled).
...
However, in open source software such as Guapcoin Core, where everyone uses
...
pay attention to for reviewers of Guapcoin Core code.

General Guapcoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Guapcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Guapcoin Core so that they can be integrated
...
file descriptors it will cause problems with Guapcoin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Guapcoin-Project/Guapcoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `guapcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `guapcoin-cli` and the GUI debug console use this table to determine how to
