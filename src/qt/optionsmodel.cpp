...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::GUAP);
    ...
    if (settings.contains("nAnonymizeGuapcoinAmount") || reset)
        SoftSetArg("-anonymizeguapcoinamount", settings.value("nAnonymizeGuapcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in guapcoin.cpp during shotdown to also remove the window positions
