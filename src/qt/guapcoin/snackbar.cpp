...
#include "qt/guapcoin/forms/ui_snackbar.h"
#include "qt/guapcoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(GuapcoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &GuapcoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
