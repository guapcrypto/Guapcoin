...
#include "qt/guapcoin/send.h"
#include "qt/guapcoin/forms/ui_send.h"
#include "qt/guapcoin/addnewcontactdialog.h"
#include "qt/guapcoin/qtutils.h"
#include "qt/guapcoin/sendchangeaddressdialog.h"
#include "qt/guapcoin/optionbutton.h"
#include "qt/guapcoin/sendconfirmdialog.h"
#include "qt/guapcoin/myaddressrow.h"
#include "qt/guapcoin/guitransactionsutils.h"
...
SendWidget::SendWidget(GuapcoinGUI* parent) :
	PWidget(parent),
	ui(new Ui::send),
	...
	btnContacts(new QPushButton())
{
	...
	/* Subtitle */
	ui->labelSubtitle1->setText(tr("You can transfer public coins (GUAP)"));
	setCssProperty(ui->labelSubtitle1, "text-subtitle");

	/* Address */
	ui->labelSubtitleAddress->setText(tr("Guapcoin address or contact label"));
	...
	ui->labelAmountSend->setText("0.00 GUAP");
	...
	setCssProperty(coinIcon, "coin-icon-guap");
...
void SendWidget::onSendClicked() {
	...
	// will call relock
	if (!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Send_GUAP, true)) {
...
void SendWidget::onCoinControlClicked() {
	if (walletModel->getBalance() > 0) {
		...
		inform(tr("You don't have any GUAP to select."));
	}
}
...
void SendWidget::onContactMultiClicked() {
	if (focusedEntry) {
		...
		CBitcoinAddress guapAdd = CBitcoinAddress(address.toStdString());
		...
		if (dialog->res) {
			...
			if (walletModel->updateAddressBookLabels(guapAdd.Get(), dialog->getLabel().toStdString(),
				AddressBook::AddressBookPurpose::SEND)) {
