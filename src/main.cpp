...
/** Fees smaller than this (in uguap) are considered zero fee (for relaying and mining)
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
	...
	if (outs >= 3 && lastOut.scriptPubKey != tx.vout[outs - 2].scriptPubKey) {
		...
		// cold staking is active much after height_start_ZC_PublicSpends so GetMasternodePayment is always 3 GUAP.
