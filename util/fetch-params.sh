...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/GuapcoinParams"
    else
        PARAMS_DIR="$HOME/.guapcoin-params"
    fi
fi
...
Failed to fetch the Guapcoin zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
Guapcoin - fetch-params.sh

This script will fetch the Guapcoin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Guapcoin zkSNARK parameters. Note that it is
